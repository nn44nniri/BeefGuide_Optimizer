...
from ligaps_graphs import generate_output_graphs
...
DEFAULT_SETTINGS_FILENAME = os.environ.get("LIGAPS_SETTINGS_FILENAME", "settings.json")
BREED_LIBRARY_PARAMETER_COUNT = 27


BREED_LIBRARY_PARAMETER_NAMES = [
    "reflectance_coat",
    "coat_length_m",
    "body_area_factor",
    "core_skin_conductance_max_W_m2_K",
    "start_body_weight_kg",
    "gompertz_max_adult_weight_kg",
    "birth_weight_kg",
    "gompertz_integration_constant",
    "gompertz_rate_constant",
    "gompertz_reduction_kg_tbw",
    "lactation_curve_parameter_a",
    "lactation_curve_parameter_b",
    "maximum_adult_weight_reference_kg",
    "sex_flag_0_male_1_female",
    "milk_available_for_calf_parameter_a",
    "milk_available_for_calf_parameter_b",
    "minimum_fraction_mature_tbw_for_gestation",
    "maintenance_correction_factor",
    "minimum_fat_tissue_fraction_for_gestation",
    "lipid_bone_parameter",
    "maximum_carcass_fraction",
    "maximum_muscle_bone_ratio",
    "minimum_core_skin_conductance_factor",
    "latent_heat_release_parameter_1",
    "latent_heat_release_parameter_2",
    "latent_heat_release_reference_temperature_c",
    "lactation_curve_parameter_c"
]
BREED_LIBRARY_PARAMETER_NAME_SET = set(BREED_LIBRARY_PARAMETER_NAMES)


def _coerce_named_breed_library(sex_value, breed_id, sex_key, missing_param_27):
    if isinstance(sex_value, dict):
        unknown = [k for k in sex_value.keys() if k not in BREED_LIBRARY_PARAMETER_NAME_SET]
        if unknown:
            raise ValueError(
                f"Breed {breed_id} {'male' if sex_key == 0 else 'female'} library contains unknown parameter names: {unknown}."
            )
        values = []
        for param_name in BREED_LIBRARY_PARAMETER_NAMES:
            if param_name == 'lactation_curve_parameter_c' and param_name not in sex_value:
                values.append(float(missing_param_27))
            elif param_name in sex_value:
                values.append(float(sex_value[param_name]))
            else:
                raise ValueError(
                    f"Breed {breed_id} {'male' if sex_key == 0 else 'female'} library is missing parameter '{param_name}'."
                )
        return np.asarray(values, dtype=float)
    library = np.asarray(sex_value, dtype=float)
    if library.ndim != 1:
        raise ValueError(f"Breed {breed_id} sex library must be a 1D list of parameters or an object with named parameters.")
    if library.size == BREED_LIBRARY_PARAMETER_COUNT - 1:
        library = np.append(library, missing_param_27)
    elif library.size != BREED_LIBRARY_PARAMETER_COUNT:
        raise ValueError(
            f"Breed {breed_id} {'male' if sex_key == 0 else 'female'} library must contain "
            f"{BREED_LIBRARY_PARAMETER_COUNT - 1} or {BREED_LIBRARY_PARAMETER_COUNT} parameters, got {library.size}."
        )
    return library
...
def _load_weather_csv(path):
    path = Path(path)
    cached = WEATHER_CACHE.get(path)
    if cached is None:
        cached = pd.read_csv(path, header=0, sep=",")
        WEATHER_CACHE[path] = cached
    return cached.copy()
...
def _load_settings_json(path=None):
    settings_path = BASE_DIR / DEFAULT_SETTINGS_FILENAME if path is None else Path(path)
    settings_path = settings_path.resolve()
    cached = SETTINGS_CACHE.get(settings_path)
    if cached is None:
        with settings_path.open("r", encoding="utf-8") as fh:
            cached = json.loads(_strip_json_comments(fh.read()))
        SETTINGS_CACHE[settings_path] = cached
    return cached


def _build_breed_library_map(settings):
    breeds = settings.get("breeds", [])
    if not isinstance(breeds, list) or not breeds:
        raise ValueError("settings.json must define a non-empty 'breeds' list.")

    defaults = settings.get("defaults", {}) if isinstance(settings, dict) else {}
    missing_param_27 = float(defaults.get("missing_breed_parameter_27", 0.00338))

    breed_library_map = {}
    breed_name_map = {}
    for breed in breeds:
        if not isinstance(breed, dict):
            raise ValueError("Each breed entry in settings.json must be an object.")
        breed_id = int(breed["id"])
        breed_name_map[breed_id] = breed.get("name", f"breed_{breed_id}")
        sex_libraries = breed.get("sex_libraries", {})
        for sex_key, sex_value in ((0, sex_libraries.get("male")), (1, sex_libraries.get("female"))):
            if sex_value is None:
                raise ValueError(f"Breed {breed_id} is missing the {'male' if sex_key == 0 else 'female'} library in settings.json.")
            library = _coerce_named_breed_library(sex_value, breed_id, sex_key, missing_param_27)
            breed_library_map[(breed_id, sex_key)] = library

    return breed_library_map, breed_name_map


def _get_breed_library_pair(breed_library_map, breed_id):
    try:
        male = breed_library_map[(int(breed_id), 0)]
        female = breed_library_map[(int(breed_id), 1)]
    except KeyError as exc:
        raise KeyError(
            f"Breed {breed_id} is not fully defined in settings.json. Both male and female libraries are required."
        ) from exc
    return male, female


DEFAULT_CASE_DEFINITIONS = [
    {"id": 1, "name": "Pot Ch Fr", "breed_id": 1, "slaughter_weight": 936.1493, "location": "FRANCE1", "housing": [0, 1, 0], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "WHEAT", "feed2_name_housed": "HAY", "feed2_name_outdoors": "HAY", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.65, "feed2_fraction": 0.35, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 2, "name": "Pot Ch Au", "breed_id": 1, "slaughter_weight": 717.3103, "location": "AUSTRALIA1", "housing": [1, 1, 1], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "WHEAT", "feed2_name_housed": "HAY", "feed2_name_outdoors": "HAY", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.65, "feed2_fraction": 0.35, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 3, "name": "Pot BxS Fr", "breed_id": 4, "slaughter_weight": 579.8809, "location": "FRANCE1", "housing": [0, 1, 0], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "WHEAT", "feed2_name_housed": "HAY", "feed2_name_outdoors": "HAY", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.65, "feed2_fraction": 0.35, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 4, "name": "Pot BxS Au", "breed_id": 4, "slaughter_weight": 559.7907, "location": "AUSTRALIA1", "housing": [1, 1, 1], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "WHEAT", "feed2_name_housed": "HAY", "feed2_name_outdoors": "HAY", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.65, "feed2_fraction": 0.35, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 5, "name": "FQlty Ch Fr", "breed_id": 1, "slaughter_weight": 877.8683, "location": "FRANCE1", "housing": [0, 1, 0], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "BARLEY", "feed2_name_housed": "HAY", "feed2_name_outdoors": "PASTURE1", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.05, "feed2_fraction": 0.95, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 6, "name": "FQlty Ch Au", "breed_id": 1, "slaughter_weight": 460.0, "location": "AUSTRALIA1", "housing": [1, 1, 1], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "BARLEY", "feed2_name_housed": "PASTURE1", "feed2_name_outdoors": "PASTURE1", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.05, "feed2_fraction": 0.95, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 7, "name": "FQlty BxS Fr", "breed_id": 4, "slaughter_weight": 574.7075, "location": "FRANCE1", "housing": [0, 1, 0], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "BARLEY", "feed2_name_housed": "HAY", "feed2_name_outdoors": "PASTURE1", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.05, "feed2_fraction": 0.95, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 8, "name": "FQlty BxS Au", "breed_id": 4, "slaughter_weight": 638.0943, "location": "AUSTRALIA1", "housing": [1, 1, 1], "feed_availability": {"feed2": 20.0, "feed3": 0.0}, "diet": {"feed1_name": "BARLEY", "feed2_name_housed": "PASTURE1", "feed2_name_outdoors": "PASTURE1", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 20.0, "feed2_quantity": 20.0, "feed3_quantity": 0.0, "feed1_fraction": 0.05, "feed2_fraction": 0.95, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 9, "name": "FQlty Ch Fr 1kg barley", "breed_id": 1, "slaughter_weight": 717.469, "location": "FRANCE1", "housing": [0, 1, 0], "feed_availability": {"feed2": 0.0, "feed3": 20.0}, "diet": {"feed1_name": "BARLEY", "feed2_name_housed": "HAY", "feed2_name_outdoors": "PASTURE1", "feed3_name_housed": "HAY", "feed3_name_outdoors": "PASTURE1", "feed1_quantity": 1.0, "feed2_quantity": 0.0, "feed3_quantity": 20.0, "feed1_fraction": 0.65, "feed2_fraction": 1.0, "feed3_fraction": 1.0, "feed4_fraction": 1.0}},
    {"id": 10, "name": "FLim Ch Fr 2pct BW", "breed_id": 1, "slaughter_weight": 992.1554, "location": "FRANCE1", "housing": [0, 1, 0], "feed_availability": {"feed2": 1.9, "feed3": 0.0}, "diet": {"feed1_name": "BARLEY", "feed2_name_housed": "HAY", "feed2_name_outdoors": "PASTURE1", "feed3_name_housed": "HAY", "feed3_name_outdoors": "HAY", "feed1_quantity": 0.1, "feed2_quantity": 1.9, "feed3_quantity": 0.0, "feed1_fraction": 0.05, "feed2_fraction": 0.95, "feed3_fraction": 1.0, "feed4_fraction": 1.0}}
]


def _build_default_scenarios_for_available_breeds(available_breed_ids):
    scenarios = json.loads(json.dumps(DEFAULT_CASE_DEFINITIONS))
    if len(available_breed_ids) == 1:
        only_breed = int(available_breed_ids[0])
        for scenario in scenarios:
            scenario["breed_id"] = only_breed
    return scenarios


def _get_settings_case_ids(settings, scenario_ids):
    simulation = settings.get("simulation", {}) if isinstance(settings.get("simulation", {}), dict) else {}
    case_ids = simulation.get("case_ids", settings.get("case_ids"))
    if case_ids is None:
        return [1, 3, 5, 7, 9, 10]
    case_ids = [int(x) for x in case_ids]
    unknown = [cid for cid in case_ids if cid not in scenario_ids]
    if unknown:
        raise KeyError(f"Unknown case_ids in settings.json: {unknown}. Available scenario ids: {sorted(scenario_ids)}")
    return case_ids


def _load_scenarios_from_settings(settings, available_breed_ids):
    raw_scenarios = settings.get("scenarios")
    if raw_scenarios is None:
        raw_scenarios = _build_default_scenarios_for_available_breeds(available_breed_ids)
    if not isinstance(raw_scenarios, list) or not raw_scenarios:
        raise ValueError("settings.json must define a non-empty 'scenarios' list.")

    scenario_map = {}
    for scenario in raw_scenarios:
        if not isinstance(scenario, dict):
            raise ValueError("Each scenario entry in settings.json must be an object.")
        sid = int(scenario["id"])
        scenario_map[sid] = scenario

    selected_case_ids = _get_settings_case_ids(settings, set(scenario_map.keys()))
    return scenario_map, selected_case_ids


def _validate_and_resolve_scenario(scenario, available_breed_ids):
    breed_id = int(scenario.get("breed_id"))
    if breed_id not in available_breed_ids:
        raise KeyError(f"Breed {breed_id} required by scenario {scenario.get('id')} is not defined in settings.json.")

    housing = scenario.get("housing", [0,1,0])
    if not isinstance(housing, list) or len(housing) != 3:
        raise ValueError(f"Scenario {scenario.get('id')} field 'housing' must contain exactly 3 values.")

    feed_availability = scenario.get("feed_availability", {})
    diet = scenario.get("diet", {})
    required_diet_keys = [
        "feed1_name", "feed2_name_housed", "feed2_name_outdoors",
        "feed3_name_housed", "feed3_name_outdoors",
        "feed1_quantity", "feed2_quantity", "feed3_quantity",
        "feed1_fraction", "feed2_fraction", "feed3_fraction", "feed4_fraction"
    ]
    missing = [k for k in required_diet_keys if k not in diet]
    if missing:
        raise ValueError(f"Scenario {scenario.get('id')} diet is missing keys: {missing}")

    return {
        "id": int(scenario["id"]),
        "name": scenario.get("name", f"case_{scenario['id']}"),
        "breed_id": breed_id,
        "slaughter_weight": float(scenario.get("slaughter_weight")),
        "location": str(scenario.get("location")),
        "housing": [int(x) for x in housing],
        "feed_availability": {
            "feed2": float(feed_availability.get("feed2", diet.get("feed2_quantity", 0.0))),
            "feed3": float(feed_availability.get("feed3", diet.get("feed3_quantity", 0.0))),
        },
        "diet": {k: (float(diet[k]) if 'quantity' in k or 'fraction' in k else str(diet[k])) for k in required_diet_keys}
    }


def _feed_matrix_by_name(feed_name, WHEAT, BARLEY, PASTURE1, HAY):
    lookup = {
        "WHEAT": WHEAT,
        "BARLEY": BARLEY,
        "PASTURE1": PASTURE1,
    ...
    key = str(feed_name).upper()
    if key not in lookup:
        raise KeyError(f"Unknown feed name '{feed_name}'. Supported names: {sorted(lookup)}")
    return lookup[key]


FEED_DISPLAY_NAMES = {"WHEAT": "Wheat", "BARLEY": "Barley", "PASTURE1": "Grass", "HAY": "Hay"}
FEED_PLOT_COLORS = {"WHEAT": "gold", "BARLEY": "seagreen", "PASTURE1": "orange", "HAY": "darkkhaki"}


def _feed_display_name(feed_name):
    key = str(feed_name).upper()
    return FEED_DISPLAY_NAMES.get(key, str(feed_name))


def _feed_plot_color(feed_name):
    key = str(feed_name).upper()
    return FEED_PLOT_COLORS.get(key, "gray")


def _build_feed_plot_components(feed1, feed2, feed3, housing, diet_cfg):
    bars = []
    labels = []
    colors = []

    def add_single(arr, name):
        arr = np.asarray(arr, dtype=float)
        if arr.size == 0 or np.allclose(arr, 0.0, equal_nan=True):
            return
        bars.append(arr)
        labels.append(_feed_display_name(name))
        colors.append(_feed_plot_color(name))

    def add_housing_split(arr, housed_name, outdoors_name, housing_vec):
        arr = np.asarray(arr, dtype=float)
        housing_vec = np.asarray(housing_vec, dtype=float)
        if arr.size == 0 or np.allclose(arr, 0.0, equal_nan=True):
            return
        housed = arr * (1.0 - housing_vec)
        outdoors = arr * housing_vec
        if np.any(np.abs(housed) > 0):
            bars.append(housed)
            labels.append(_feed_display_name(housed_name))
            colors.append(_feed_plot_color(housed_name))
        if np.any(np.abs(outdoors) > 0):
            bars.append(outdoors)
            labels.append(_feed_display_name(outdoors_name))
            colors.append(_feed_plot_color(outdoors_name))

    add_single(feed1, diet_cfg['feed1_name'])
    if str(diet_cfg['feed2_name_housed']).upper() == str(diet_cfg['feed2_name_outdoors']).upper():
        add_single(feed2, diet_cfg['feed2_name_outdoors'])
    else:
        add_housing_split(feed2, diet_cfg['feed2_name_housed'], diet_cfg['feed2_name_outdoors'], housing)

    if str(diet_cfg['feed3_name_housed']).upper() == str(diet_cfg['feed3_name_outdoors']).upper():
        add_single(feed3, diet_cfg['feed3_name_outdoors'])
    else:
        add_housing_split(feed3, diet_cfg['feed3_name_housed'], diet_cfg['feed3_name_outdoors'], housing)

    if not bars:
        bars = [np.zeros_like(np.asarray(feed1, dtype=float))]
        labels = ["No feed"]
        colors = ["gray"]

    return np.vstack(bars), labels, colors
...
# the paper. These defaults can now be overridden dynamically from settings.json.

SETTINGS = _load_settings_json()
breed_library_map, breed_name_map = _build_breed_library_map(SETTINGS)
available_breed_ids = sorted({breed_id for breed_id, _sex in breed_library_map.keys()})
scenario_map, case_ids = _load_scenarios_from_settings(SETTINGS, available_breed_ids)
...
# Case execution is now controlled from settings.json.
for z in progress_iter(case_ids, total=len(case_ids), desc="Cases", position=0, leave=PROGRESS_LEAVE_OUTER ):
    scenario = _validate_and_resolve_scenario(scenario_map[z], available_breed_ids)
    # z-loop for each configured scenario. The ids correspond to the original illustration
    # case numbering unless customized in settings.json.
    ...
    for s in progress_iter( sens_ids, total=len(sens_ids), desc=f"Case {z} / sensitivity", position=1, leave=False ):
        ...
        BREED = int(scenario['breed_id'])  # Breed identifier; libraries are loaded from settings.json
        if (BREED, 0) not in breed_library_map or (BREED, 1) not in breed_library_map:
            available_txt = ", ".join(str(bid) for bid in available_breed_ids)
            raise KeyError(
                f"Breed {BREED} required by case {z} is not defined in settings.json. Available breed ids: {available_txt}."
            )
        # Montana; 4 = Brahman (3/4) x Shorthorn (1/4); 5 =
        # Hereford (only steers)

        LOCATION = scenario['location']    # See settings.json for the geographical location.
        ...
        PHASE1 = np.repeat(scenario['housing'][0], 84)   # Housing period 1 (indicates January - March;
        # 25th of March = day 84)
        PHASE2 = np.repeat(scenario['housing'][1], 260)  # Housing period 2 (indicates March - December)
        PHASE3 = np.repeat(scenario['housing'][2], 21)   # Housing period 3 (indicates December)
        ...
        SWMALES = scenario['slaughter_weight'] # Slaughter weight male calf/calves (kg)
        ...
        if LOCATION == "FRANCE1":
            ...
            WEATHER = _load_weather_csv(weather_file)
        else:
        ...
        # Modify weather data if cattle are housed in stables or feedlots
        housed_mask = HOUSING[:imax] == 0
        if np.any(housed_mask):
            weather_view = WEATHER.iloc[:imax].copy()
            weather_view.loc[housed_mask, "RAD"] = weather_view.loc[housed_mask, "RAD"] * RADTRANS
            weather_view.loc[housed_mask, "WIND"] = weather_view.loc[housed_mask, "WIND"] * WINDRED
            weather_view.loc[housed_mask, "MINT"] = Tdelta * weather_view.loc[housed_mask, "MINT"] + TINCR
            weather_view.loc[housed_mask, "MAXT"] = Tdelta * weather_view.loc[housed_mask, "MAXT"] + TINCR
            WEATHER.iloc[:imax] = weather_view
        ...
        ###########################################################################################
        # Breed genotype libraries are loaded dynamically from settings.json.
        # Each breed in settings.json must define a complete male and female
        # parameter library with 27 values, matching the original LiGAPS-Beef
        # library layout documented in the comments above.
        ...
        diet_cfg = scenario['diet']
        seasonal_mask = (
            ((TIMESTEPS >= 1) & (TIMESTEPS <= 84)) |
            ((TIMESTEPS >= 345) & (TIMESTEPS <= 449)) |
            ((TIMESTEPS >= 710) & (TIMESTEPS <= 814)) |
            ((TIMESTEPS >= 1075) & (TIMESTEPS <= 1179)) |
            (TIMESTEPS > 1439)
        )

        F1[:, :] = _feed_matrix_by_name(diet_cfg['feed1_name'], WHEAT, BARLEY, PASTURE1, HAY)
        F2[:, :] = _feed_matrix_by_name(diet_cfg['feed2_name_outdoors'], WHEAT, BARLEY, PASTURE1, HAY)
        F2[seasonal_mask, :] = _feed_matrix_by_name(diet_cfg['feed2_name_housed'], WHEAT, BARLEY, PASTURE1, HAY)
        F3[:, :] = _feed_matrix_by_name(diet_cfg['feed3_name_outdoors'], WHEAT, BARLEY, PASTURE1, HAY)
        F3[seasonal_mask, :] = _feed_matrix_by_name(diet_cfg['feed3_name_housed'], WHEAT, BARLEY, PASTURE1, HAY)

        FEED1 = F1
        FEED2 = F2
        FEED3 = F3

        FEED1QNTY.fill(float(diet_cfg['feed1_quantity']))
        FEED2QNTY.fill(float(scenario['feed_availability']['feed2']))
        FEED3QNTY.fill(float(scenario['feed_availability']['feed3']))
        # orginal
        # Selection of feed types and feed quantities over simulation time if a specific diet
        # (1-5) is chosen
        ...
        # Fractions of feed types in the diet over time are now configurable per scenario.
        FEED1fr = float(diet_cfg['feed1_fraction'])
        FEED2fr = float(diet_cfg['feed2_fraction'])
        FEED3fr = float(diet_cfg['feed3_fraction'])
        FEED4fr = float(diet_cfg['feed4_fraction'])
        ...
        for j in progress_iter( range(1, jmax + 1), total=jmax, desc=f"Case {z} / animals", position=2, leave=False, enable=SHOW_PROGRESS and (not DEBUG_LOOP) ):
            ...
            LIBRARY = breed_library_map[(int(BREED), int(SEX[j - 1]))].copy()
            ...
            weather_mint = WEATHER["MINT"].to_numpy()
            weather_maxt = WEATHER["MAXT"].to_numpy()
            weather_vpr = WEATHER["VPR"].to_numpy()
            weather_wind = WEATHER["WIND"].to_numpy()
            weather_rain = WEATHER["RAIN"].to_numpy()
            weather_okta = WEATHER["OKTA"].to_numpy()
            weather_aha = WEATHER["AHA"].to_numpy()
            weather_rad = WEATHER["RAD"].to_numpy()
            ...
            for i in progress_iter( range(1, imax_j + 1), total=imax_j, desc=f"Case {z} / animal {j} / days", position=3, leave=PROGRESS_LEAVE_INNER, enable=SHOW_PROGRESS and (not DEBUG_LOOP) ):
                ...
                TAVGC[i - 1, j - 1] = (weather_mint[i - 1] +
                       weather_maxt[i - 1]) / 2
                ...
                VPAIRTOT[i - 1, j - 1] = weather_vpr[i - 1] * 1000
                # relative humidity (-)
                ...
                DLC[i - 1, j - 1] = (
                    CoatConst * weather_wind[i - 1]
                ) / (
                    (CoatConst * weather_wind[i - 1]) / LC + 1 / (ZC * LC)
                )
                ...
                CSC[i - 1, j - 1] = CSC[i - 1, j - 1] / (
                    1 - min(RAINFRAC, weather_rain[i - 1] * RAINFRAC / 24)
                )
                ...
                Ea[i - 1, j - 1] = weather_vpr[i - 1] * 10

                # Calculation of the Reynolds number
                # Wind speed (m s-1)
                WINDSP[i - 1, j - 1] = weather_wind[i - 1]
                # Reynolds number
                REYNOLDS[i - 1, j - 1] = (
                ...
                SAAC[i - 1, j - 1] = weather_aha[i - 1]
                # Incoming direct solar radiation on the soil surface (Wm-2)
                SWRS[i - 1, j - 1] = weather_rad[i - 1] * kJdaytoW
                ...
                RAINEVAP[i - 1, j - 1] = (
                    ...
                    AREA[i - 1, j - 1] *
                    min(24, weather_rain[i - 1]) * L * kJdaytoW
                )
                ...
                breed_library_male, breed_library_female = _get_breed_library_pair(breed_library_map, BREED)
                calf_sex = int(SEX[calf_idx])
                BIRTHW1[i - 1, j - 1] = breed_library_male[4] - calf_sex * (breed_library_male[4] - breed_library_female[4])
                ...
                calf_idx = int(CALFNR[i - 1, j - 1])
                calf_sex = int(SEX[calf_idx])
                calf_library = breed_library_map[(int(BREED), calf_sex)]
                NEREQGESTTOT = 58.658 * calf_library[6] + 0.5502
...
# } for the time loop
    ...
    (
        ...
        FEED2QNTY=FEED2QNTY,
        FEED3QNTY=FEED3QNTY,
        FEED4QNTY=locals().get("FEED4QNTY"),
        HOUSING1=locals().get("HOUSING1"),
        HOUSING=HOUSING,
        ENDDAY=ENDDAY,
        REDHP=REDHP,
        ...
        FEEDQNTY=FEEDQNTY,
        diet_cfg=diet_cfg,
        build_feed_plot_components=_build_feed_plot_components,
        GENLIMdata=GENLIMdata,
        HEATSTRESSdata=HEATSTRESSdata,
        COLDSTRESSdata=COLDSTRESSdata,
        FILLGITGRAPHdata=FILLGITGRAPHdata,
        NELIMdata=NELIMdata,
        PROTGRAPHdata=PROTGRAPHdata,
    )
    ...
    # Bull calf end day used below for the slaughter-weight summary entry.
    ...
    output_json_path = os.environ.get("LIGAPS_OUTPUT_JSON")
    if output_json_path:
        summary_payload = {
            "case_number": int(z),
            "feed_efficiency_herd_g_beef_per_kg_dm": float(TABLEDATA.iloc[0, 0]),
            "feed_efficiency_repr_cow_g_beef_per_kg_dm": float(TABLEDATA.iloc[1, 0]),
            "feed_efficiency_bull_calf_g_beef_per_kg_dm": float(TABLEDATA.iloc[2, 0]),
            "feed_fraction_repr_cow": float(TABLEDATA.iloc[3, 0]),
            "beef_production_herd_kg": float(TABLEDATA.iloc[4, 0]),
            "beef_production_repr_cow_kg": float(TABLEDATA.iloc[5, 0]),
            "beef_production_bull_calf_kg": float(TABLEDATA.iloc[6, 0]),
            "slaughter_weight_bull_calf_kg": float(TABLEDATA.iloc[7, 0])
        }
